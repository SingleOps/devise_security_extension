...
Warden::Manager.after_set_user :except => :fetch do |record, warden, options|
  if record.respond_to?(:update_unique_session_id!) && record.responds_to?(:should_limit_sessions?) && record.should_limit_sessions? && warden.authenticated?(options[:scope])
...
Warden::Manager.after_set_user :only => :fetch do |record, warden, options|
  ...
  if record.respond_to?(:unique_session_id) && warden.authenticated?(scope) && options[:store] != false
    if record.unique_session_id != warden.session(scope)['unique_session_id'] && record.responds_to?(:should_limit_sessions?) && record.should_limit_sessions? && !env['devise.skip_session_limitable']
